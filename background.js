  ...
  browser.webRequest.onBeforeSendHeaders.addListener(
    function(details) {
      details.requestHeaders.push({
        name: `X-Flavortown-Ext-${FLAVORTOWN_PROJECT_ID}`,
        value: 'true'
      });
...
function saveCurrentTime() {
  console.log('💾 saveCurrentTime called, currentSite:', currentSite, 'startTime:', startTime, 'focusSessionActive:', focusSessionActive);
  if (!currentSite || !startTime) {
    console.log('⚠️ Skipping save - no currentSite or startTime');
    return;
  }
  
  const timeSpent = Date.now() - startTime;
  console.log('⏱️ Time spent:', timeSpent, 'ms (', Math.round(timeSpent / 1000), 'seconds ) on', currentSite);
  ...
  browserAPI.storage.local.get(['dailyData', 'weeklyData', 'monthlyData', 'categories', 'focusState'], (result) => {
    ...
    // 🔥 FIX: Check BOTH focusSessionActive AND focusState from storage
    const isFocusActive = focusSessionActive || (focusState.active && !focusState.isBreak);
    
    console.log('🔍 Focus check - focusSessionActive:', focusSessionActive, 'focusState.active:', focusState.active, 'focusState.isBreak:', focusState.isBreak, 'isFocusActive:', isFocusActive);
    
    if (isFocusActive) {
      dailyData[today].study += seconds;
      weeklyData[weekKey].study += seconds;
      monthlyData[monthKey].study += seconds;
      console.log('📚✅ FOCUS SESSION ACTIVE - COUNTED AS STUDY:', seconds, 'seconds');
    } else if (categories[currentSite] === 'study') {
      dailyData[today].study += seconds;
      weeklyData[weekKey].study += seconds;
      monthlyData[monthKey].study += seconds;
      console.log('📚 Site categorized as study:', seconds);
      ...
      console.log('🎮 Entertainment time:', seconds);
    }
    
    console.log('💾 Saving data - study time for today:', dailyData[today].study);
    browserAPI.storage.local.set({ dailyData, weeklyData, monthlyData });
  });
}

// ⏱️ IMPROVED: Focus session timer with milliseconds
function startFocusTimer(state) {
  ...
  // Initialize milliseconds if not present
  if (typeof state.milliseconds === 'undefined') {
    state.milliseconds = 0;
  }
  
  // Millisecond timer - updates 10 times per second for smooth display
  millisecondInterval = setInterval(() => {
    browserAPI.storage.local.get(['focusState'], (result) => {
      const focusState = result.focusState;
      
      if (!focusState || !focusState.active || focusState.paused) {
        return;
      }
      
      focusState.milliseconds = (focusState.milliseconds || 0) + 100;
      
      if (focusState.milliseconds >= 1000) {
        focusState.milliseconds = 0;
      }
  ...
  focusTimerInterval = setInterval(() => {
    browserAPI.storage.local.get(['focusState'], (result) => {
      ...
      focusState.milliseconds = 0; // Reset milliseconds on each second
      console.log('⏱ Background timer tick:', focusState.timeRemaining, 'isBreak:', focusState.isBreak);
      
      // 🔥 FIX: Save time every second during focus sessions (not just every 30 seconds)
      ...
      if (focusState.timeRemaining <= 0) {
        if (focusState.isBreak) {
          ...
          // Focus session ended - SAVE TIME BEFORE ENDING
          saveCurrentTime();
          ...
          // Start fresh tracking for break period
          startNewTracking();
...
browserAPI.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'focusSession') {
    ...
    if (request.state === 'start') {
      ...
      clearInterval(focusTimerInterval);
      clearInterval(millisecondInterval);
      browserAPI.storage.local.set({ focusState: { active: false, milliseconds: 0 } }, () => {
  ...
  if (request.action === 'getFocusState') {
    browserAPI.storage.local.get(['focusState', 'focusSessions'], (result) => {
      sendResponse({ 
        focusState: result.focusState || { active: false, milliseconds: 0 },
